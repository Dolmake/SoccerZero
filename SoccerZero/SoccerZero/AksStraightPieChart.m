...
- (void)addDataToRepresent:(int)value WithColor:(UIColor *)color {
    ...
    } else NSLog(@"\nHey...you are passing invalid data to the aks straight pie chart...please correct it: %i.", value);
}
...
-(void) fillWithPercent:(CGFloat)percent WithColor:(UIColor*) color{
    [self addDataToRepresent:(percent*100) WithColor:color];
}

- (void) fillWithPercent:(CGFloat)percent WithColor:(UIColor *)color BackColor:(UIColor*)backColor{
    [self clearChart];
    [self addDataToRepresent:(percent*100) WithColor:color];
    [self addDataToRepresent:((1.0f- percent) * 100) WithColor:backColor];
}
